...
for (const outfit of outfits) {
    ...
    nombre.innerHTML = `<h5 class="card-title text-center"> ${outfit.nombre}</h5>`
    info.appendChild(nombre)
    let precio = document.createElement("h5")
    precio.innerHTML = `<h5 class="card-title text-center"> $ ${outfit.precio}</h5>`
    info.appendChild(precio)
    let fin = document.createElement("a")
    fin.innerHTML = `<a id=${outfit.ref} class="btn btn-outline-dark w-100 btn__carrito">Agregar a Carrito</a>`
