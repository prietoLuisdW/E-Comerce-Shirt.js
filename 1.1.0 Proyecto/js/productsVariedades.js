...
for (const variedad of variedades) {
    ...
    nombre.innerHTML = `<h5 class="card-title text-center"> ${variedad.nombre}</h5>`
    info.appendChild(nombre)
    let precio = document.createElement("h5")
    precio.innerHTML = `<h5 class="card-title text-center"> $ ${variedad.precio}</h5>`
    info.appendChild(precio)
    let fin = document.createElement("a")
    fin.innerHTML = `<a id=${variedad.ref} class="btn btn-outline-dark w-100 btn__carrito">Agregar a Carrito</a>`
