...
for (const novedad of novedades) {
    ...
    nombre.innerHTML = `<h5 class="card-title text-center"> ${novedad.nombre}</h5>`
    info.appendChild(nombre)
    let precio = document.createElement("h5")
    precio.innerHTML = `<h5 class="card-title text-center"> $ ${novedad.precio}</h5>`
    info.appendChild(precio)
    let fin = document.createElement("a")
    fin.innerHTML = `<a id=${novedad.ref} class="btn btn-outline-dark w-100 btn__carrito">Agregar a Carrito</a>`
